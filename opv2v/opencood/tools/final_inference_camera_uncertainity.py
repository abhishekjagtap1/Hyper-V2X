...
def camera_inference_visualization_uncertainty(output_dict,
    ...
    output_folder = os.path.join(output_dir, 'test_vis_K100')
    ...
    if model_type == 'dynamic':
        ...
        if pred_uncertainty is not None:
            # 1. Extract & collapse class dim
            pred_uncertainty = pred_uncertainty[0, 0].cpu().numpy()  # [C,H,W]
            pred_uncertainty = pred_uncertainty.mean(axis=0)  # [H,W]

            # 2. Match resolution with dynamic_map exactly
            ...
            # 3. Normalize and colorize
            max_val = pred_uncertainty.max()
            if max_val > 0:
                pred_uncertainty = pred_uncertainty / max_val
            pred_uncertainty = (pred_uncertainty * 255).astype(np.uint8)
            uncertainty_rgb = cv2.applyColorMap(pred_uncertainty, cv2.COLORMAP_JET)

            # 4. Optional: overlay with predicted map for alignment
            pred_map_vis = output_dict['dynamic_map'][0].cpu().numpy()
            pred_map_vis = np.uint8(pred_map_vis * 127)
            pred_map_rgb = cv2.cvtColor(pred_map_vis, cv2.COLOR_GRAY2BGR)
            overlay = cv2.addWeighted(pred_map_rgb, 0.4, uncertainty_rgb, 0.6, 0)

            # 5. Resize once for display
